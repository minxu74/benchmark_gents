...
drc_out="${DATA}/ne30/clm_test" # Native grid output directory
...
if [ ! -d ${drc_tmp}_${runtime} ]; then
   mkdir -p ${drc_tmp}_${runtime}
fi

if [ ! -d ${drc_out}_${runtime} ]; then
   mkdir -p ${drc_out}_${runtime}
fi

/bin/rm -f ${drc_out}_${runtime}/*.nc
...
export TMPDIR=${drc_tmp}_${runtime}
#/bin/rm ~/ncremap.lnd
...
{ time /bin/ls *.clm2.h0.25[78][0-9]-*.nc *.clm2.h0.259[0-6]-*.nc | ncclimo -c mycase --var=${vars} --job_nbr=136 ${cmip6_opt} ${spl_opt} --drc_out=${drc_out}_${runtime} >> ~/ncclimo.lnd_${runtime} 2>&1 ;} 2> $workdir/time_${runtime}.txt
# Regrid Land
